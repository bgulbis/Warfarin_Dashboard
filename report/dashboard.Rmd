...
warf.doses <- data.warfarin %>%
    ...
    group_by(pie.id, warf.year) %>%
    mutate(cumdose = cumsum(dose))
...
# library(rbokeh)
# figure() %>%
#     ly_boxplot(x = factor(year), y = dose, data = first.dose)
...
```{r}
df.inr.dose <- full_join(warf.doses, data.coags, by = c("pie.id", "warf.year", "warf.day")) %>%
    left_join(ds.pts, by = "pie.id") %>%
    mutate(group = if_else(pharmacy == TRUE, "pharmacy", "traditional", "traditional"),
           year = as.character(year(warf.year))) %>%
    filter(cumdose <= 250)
...
ggplot(df.inr.dose, aes(x = cumdose, y = inr, color = group)) +
