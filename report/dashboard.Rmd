...
plot_ly(data = first.dose, x = ~factor(year), y = ~dose, color = ~group, colors = "Set1") %>%
    add_boxplot()
```
...
# library(rbokeh)
# figure(d) %>%
#     ly_points(x = warf.day, y = inr.mean, glyph = group, color = year, hover = c(group, warf.day, inr.mean)) %>%
#     ly_lines(x = warf.day, y = inr.mean, group = group, color = year)

```

```{r}
df <- group_by(d, group, year)
